...
for(jj in 1:length(epsilon.vec)){
  ...
  IAR.mosquito[,jj] = optim(c(S0,S0),function(par)
...
frac.bias.human <-  (efficacy.bestcase - efficacy.human) / (efficacy.bestcase - efficacy.mosquito)
frac.bias.fullmodel <- (efficacy.human - efficacy.fullmodel) / (efficacy.bestcase - efficacy.mosquito)
frac.bias.mosquito <- 1 - frac.bias.fullmodel - frac.bias.human
