...
calc_efficacy <- function(rho.tt, # proportion of time that treated individuals spend in treated clusters
                          rho.cc, # proportion of time that control individuals spend in control clusters
                          Ct, # coverage in treated clusters
                          Cc, # control in control clusters
                          FOI, # force of infection
                          epsilon, # factor reduction in R0 from intevention
                          age.dist, # age distribution for Indonesia
                          ...
                          population_structure = 'exponential') # assumption about population structure for R0 calculation. can be "exponential" or "rectangular"
 {
  ...
  if(population_structure == 'rectangular')
  {
  ...
  IAR.suppression <- optim(c(0.9,0.9),function(par)
    loss.two(par[1],par[2], rho.tt = rho.tt, rho.tc = rho.tc, rho.cc = rho.cc, rho.ct = rho.ct, Cc = Cc, Ct = Ct, Nt = Nt, Nc = Nc, epsilon = epsilon, R0 = R0),lower=c(0,0),upper=c(1,1),method='BFGS',
